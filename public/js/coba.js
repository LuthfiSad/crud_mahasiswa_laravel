...
function darkMode(el) {
  ...
  const qlTools = document.querySelector('#editor .ql-tooltip');
  const btnCloseInvert = document.querySelectorAll('form span.input-group-text button.btn-close, .modal button.btn-close');

  if (!el.getAttribute("checked")) {
    body.classList.add('dark-version');
    localStorage.setItem("mode", "dark");
    if (btnCloseInvert.length == !0) {
      for (var i = 0; i < btnCloseInvert.length; i++) {
        btnCloseInvert[i].setAttribute('style', `filter: invert(0);`);
      }   
    }
    console.log(btnCloseInvert[1].setAttribute('style', `filter: invert(0);`));
    if(qlTools == !null){
      qlTools.classList.add('bg-dark', 'text-white');
    }
    if (navbarBrandImg.includes('logo-ct-dark.png')) {
      var navbarBrandImgNew = navbarBrandImg.replace("logo-ct-dark", "logo-ct");
      navbarBrand.src = navbarBrandImgNew;
    ...
    for (var i = 0; i < pagination.length; i++) {
        pagination[i].classList.add('bg-dark');
    }
    ...
    if (btnCloseInvert === !0) {
      for (var i = 0; i < btnCloseInvert.length; i++) {
        btnCloseInvert[i].setAttribute('style', `filter: invert(.6);`);
      }
    }
    if(qlTools == !null){
      qlTools.classList.remove('bg-dark', 'text-white');
    }
    if (navbarBrandImg.includes('logo-ct.png')) {
      var navbarBrandImgNew = navbarBrandImg.replace("logo-ct", "logo-ct-dark");
      navbarBrand.src = navbarBrandImgNew;
    ...
    for (var i = 0; i < pagination.length; i++) {
      pagination[i].classList.remove('bg-dark');
    }
...
const toggleMode = document.querySelector('#dark-version');
if(getMode === "dark"){
